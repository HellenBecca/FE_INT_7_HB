...
var age = prompt("Please enter your age"); // Checks if input is null, empty or left the input empty (including spaces)
...
var specialCharacters = ["!", "@", "#", "$", "%", "&", "/", "(", ")", "="]; // Starts a for loop to allow the user up to 3 attempts to input a valid number

for (var attempts = 0; attempts < 3; attempts++) {
  // Ask the user to enter a number between 0 and 9
  var number = prompt("Enter a number from 0 to 9:"); // Checks if the user clicked "Cancel" or left the input empty (including spaces)
  ...
  number = Number(number); // Validates the input: check if it is a number and within the range 0-9
  ...
  if (!isNaN(number) && number >= 0 && number <= 9) {
    ...
    alert("Invalid input. Please enter a number between 0 and 9."); // If this is the last attempt (attempts === 2), the user is informed
...
var low = 0; // Lowest possible number

var high = 100; // Highest possible number

var guess = Math.floor((low + high) / 2); // Initial guess is the middle of the range
// The loop for guessing

while (true) {
  // Asking the user for feedback about the guess
  var response = prompt("Is your number > ".concat(guess, ", < ").concat(guess, ", or == ").concat(guess, "? (Enter >, <, or ==)")); // Step-by-step simulation if the user thinks of the number, let say 40:
  ...
  if (response === ">") {
    ...
    alert("Great! The number is ".concat(guess, "."));
    break;
  } else {
